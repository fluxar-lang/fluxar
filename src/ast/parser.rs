...
impl Parser {
	...
	fn class_declaration(&mut self) -> Result<Statement, String> {
		let name =
			self.consume(Identifier, "Expected name after 'class' keyword.")?;
		let mut generics = Vec::new();
		if self.match_token(TokenType::Less) {
			loop {
				generics.push(
					self.consume(Identifier, "Expected type parameter.")?,
				);
	...
	fn function(&mut self, kind: FunctionKind) -> Result<Statement, String> {
		let name =
			self.consume(Identifier, &format!("Expected {kind:?} name"))?;
		if self.match_token(Gets) {
			...
			return Ok(Statement::CmdFunction {
				name,
				cmd: cmd_body.lexeme,
			});
		}
		...
		let body = match self.block_statement()? {
			...
			_ => {
				panic!("Block statement parsed something that was not a block")
			}
		};
	...
	fn function_expression(&mut self) -> Result<Expr, String> {
		let paren =
			self.consume(LeftParen, "Expected '(' after anonymous function")?;
		let mut generics = Vec::new();
		if self.match_token(Less) {
			loop {
				generics.push(
	...
	fn call(&mut self) -> Result<Expr, String> {
		...
		loop {
			if self.match_token(LeftParen) {
				...
				let name = self
					.consume(Identifier, "Expected token after dot-accessor")?;
				expr = Get {
					id: self.get_id(),
					object: Box::new(expr),
					name,
