...
impl Resolver {
	...
	fn resolve_internal(
		...
		match statement {
				...
				if let Some(super_expr) = superclass {
					...
					} = super_expr
					{
						if super_name.lexeme == name.lexeme {
							return Err("A class cannot inherit from itself"
								.to_string());
						}
					}
			...
			Statement::IfStmt {
				...
				then: _,
				els: _,
			} => self.resolve_if_stmt(statement)?,
			...
			Statement::CmdFunction { name: _, cmd: _ } => {
				self.resolve_var(statement)?
			}
		}
		Ok(())
	...
	fn resolve_many(
		...
		for statement in statements {
			self.resolve_internal(statement)?;
		}
		Ok(())
	}
	...
	fn resolve_if_stmt(&mut self, statement: &Statement) -> Result<(), String> {
		...
		} = statement
		{
			...
			panic!("Wrong type in resolve if statement");
		}
	}
	...
	fn declare(&mut self, name: &Token) -> Result<(), String> {
		...
		if self.scopes[size - 1].contains_key(&name.lexeme.clone()) {
			return Err(
				"A variable with this name is already in scope".to_string()
			);
		};
		...
		match expr {
			...
			Expr::This { id: _, keyword } => {
				if self.current_function != FunctionType::Method {
					return Err("Cannot use 'this' keyword outside of a class"
						.to_string());
				};
			...
			Expr::Super {
				...
				if self.current_function != FunctionType::Method {
					return Err(
						"Cannot use 'super' keyword outside of a class"
							.to_string(),
					);
				}
			...
			Expr::Variable {
				...
				var_type: _,
				name: _,
			} => self.resolve_expr_var(expr, expr.get_id()),
			Expr::Unary {
				id: _,
				operator: _,
				right,
