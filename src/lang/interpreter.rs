...
use crate::expr::{
	CallableImpl, FluxarFunctionImpl, LiteralValue, NativeFunctionImpl,
};
		...
		for stmt in stmts {
			match stmt {
				...
				Statement::Function {
					...
					let fun = LiteralValue::Callable(
						CallableImpl::FluxarFunction(callable),
					);
				...
				Statement::Class {
					...
					if let Some(superclass) = superclass {
						let superclass =
							superclass.evaluate(self.environment.clone())?;
					...
					for generic in generics {
						self.environment
							.define(generic.lexeme.clone(), LiteralValue::Nil);
					}
					...
					self.environment =
						*self.environment.enclosing.clone().unwrap();
				}
				Statement::IfStmt {
					predicate,
					then,
	...
	fn make_function(&self, fn_stmt: &Statement) -> FluxarFunctionImpl {
		...
		} = fn_stmt
		{
			...
			let params: Vec<Token> =
				params.iter().map(|t| (*t).clone()).collect();
